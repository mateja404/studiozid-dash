...
const HomePage = () => {
  ...
  const [changes, setChanges] = useState<ChangesInterface[]>([]);

  useEffect(() => {
      async function getProjectDetails() {
        await axios.get(`/api/get-project-details/${id}`)
        .then(res => {
            console.log(res.data.projectRows);
            setProject(res.data.projectRows);
            setStatus(res.data.projectRows[0].finished);
            setLng(res.data.projectRows[0].lng);
            setLat(res.data.projectRows[0].lat);
        });
      }
      async function getProjectChanges() {
        try {
          const res = await axios.get(`/api/get-projects-changes-by-id/${id}`);
          setChanges(res.data.projectChanges);
        } catch (error) {
          console.log(error);
        }
      }
  ...
  async function changeProjectStatus() {
    try {
      const res = await axios.patch(`/api/finish-project/${id}`);
          ...
          <div className={`flex-grow flex flex-col p-4 max-sm:mt-10 md:mt-25 lg:mt-0 xl:mt-0 2xl:mt-0 sm:mt-30 md:p-7 lg:pl-[220px] xl:pl-[320px] 2xl:pl-[320px] max-sm:pt-[120px] ${isOpen ? "hidden" : ""}`}>
              <div className='w-full mb-8'>
                  ...
                  <div className='mt-7 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
                      ...
                      <Button className="text-white pt-1 pb-1 pl-5 pr-5 rounded-full bg-black font-semibold cursor-pointer z-0" onClick={changeProjectStatus} disabled={status === "Završen"}> Označi kao završeno</Button>
                  </div>
                {status === "utoku" ? (
                        ...
                        <ul key={index}>
                          <li className="relative flex items-start">
                            ...
                            <div className="ml-8 flex flex-col space-y-2">
                              <h3 className="text-lg font-semibold">{new Date(change.change_date).toLocaleDateString("sr-RS")}</h3>
                              <p className="text-gray-700">{change.change_description}</p>
                              <span className="text-sm text-gray-500">{new Date(change.change_date).toLocaleTimeString("sr-RS", { hour: "2-digit", minute: "2-digit", })}</span>
                            </div>
                          </li>
                        </ul>
                      ))}
