...
const SidebarNew = ({ isOpen }: any) => {
    return (
        <nav>
            {/* Desktop Sidebar */}
            <div className='fixed left-0 top-0 bg-white shadow-md rounded-xl hidden lg:flex xl:flex 2xl:flex lg:w-[210px] xl:w-[300px] 2xl:w-[300px] h-screen z-20'>
            ...
            {/* Mobile Sidebar */}
            <div className={`fixed top-0 left-0 h-[100px] w-full z-0 bg-white shadow-md transition-all duration-300 ${isOpen ? 'min-h-screen flex flex-col' : 'h-[60px] flex justify-center items-center'}`}>
                <h1 className='absolute left-7 font-bold text-2xl mt-5'>Studio Zid</h1>
                <ul className={`transition-all duration-300 mt-24 flex flex-col items-center gap-y-7 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
